...
| `performance_delta` | Secondary target (kg): `top_set_e1rm_kg − e1rm_trend`, where `e1rm_trend` = mean of prior **3 same-exercise** top-set e1RMs (current session excluded). **Not** vs last session's 1RM alone. The **primary** readiness **class** (below/at/above trend) is derived from this — see [`feature-engineering.md`](feature-engineering.md#performance_delta-kg) |
