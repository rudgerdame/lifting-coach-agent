...
def _write_univariate_plots(
    ...
    *,
    ...
    for name, _ in shap_ranked:
        if name in CATEGORICAL_COLS or name not in merged.columns:
            continue
...
def _build_report(
    ...
    data_caveat = (
        "- Synthetic data — metrics are illustrative until real logs are used.\n"
        if is_synthetic
        else "- Trained on personal logs — do not commit artifacts or reports with identifiable data.\n"
    )
